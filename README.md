...
- **ALVAREZ GUEVARA LAURA JOCELYN**
- **PEREZ BAUTISTA ALINE BRISEIDA**
- **CRUZ VICTORIANO EMMANUEL**
...
- **Tailwind CSS v4**
- **MySQL** (local o via Docker)

## Requisitos previos

- Node.js 20 o superior
- MySQL corriendo en tu sistema (local o Docker)
...
**Opción A — MySQL local (si ya tienes MySQL instalado):**
...
**Opción B — Docker (si no tienes MySQL instalado):**

```bash
docker run -d --name real_inventado \
  -e MYSQL_ROOT_PASSWORD=password \
  -e MYSQL_DATABASE=realInventado_db \
  -p 3306:3306 mysql:8
```

Comprueba que el contenedor corre correctamente:
...
Reemplaza `TU_PASSWORD` con la contraseña de tu MySQL. Para generar una clave secreta segura:
...
3. Ve el feedback de cada respuesta con su explicación
4. Al terminar las 5 preguntas verás tu resultado final

### Panel de administración

Para acceder al panel de gestión necesitas rol de admin. Ejecuta esto reemplazando `TU_USUARIO` con el username que registraste:
...
└── seed.ts               # Datos de prueba (50 preguntas)
...
| `npx playwright test` | Corre las pruebas E2E                  |
